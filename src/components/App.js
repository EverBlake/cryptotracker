...
class App extends Component {
    ...
    for(let j=0; j<ccArray.length; j++) {
      ...
      for(let i=0; i<coins.length; i++) {
        ...
        if(coins[i].name === ccArray[j].name) {
          //add currency to the current API data
  ...
  axios({
    ...
    "headers":{
      ...
      "x-rapidapi-host":"coinpaprika.p.rapidapi.com",
      "x-rapidapi-key":"62325b0e0dmshd0398453ba5dca1p19ee54jsnec1dd8adafd9",
      "useQueryString":true
    }
  })
  .then((response) => {
                      ...
                      <tbody>
                        {this.state.ccData.map((data, key) => {
                          return(
                            <tr key={key}>
                            <td>{data.rank}</td>
                            <td><img src={data.img} width="25" height="25" className="d-inline-block align-top" alt=""></img></td>
                            <td><a target="blank" rel="noopener nonreferrer" href={"https://coinpaprika.com/coin/" + data.id}>{data.name}</a></td>
